...
export class RoleSyncService {
  ...
  async syncMember(guild: Guild, memberParam: GuildMember, userLink: UserLink): Promise<void> {
    ...
    try {
      ...
      // Citizen Role: The bot assigns this to every verified member during sync.
      // It is a managed role, not a prerequisite.
      if (config.citizenRoleId) {
        managedRoleIds.add(config.citizenRoleId);
        targetRoleIds.add(config.citizenRoleId);
      }

      // Trusted Role: An OPTIONAL external gate assigned manually by admins.
      // If configured and the member does NOT have it, skip government/leadership roles only.
      // All other managed roles (MU, level, specialization, citizen) are still assigned.
      const isTrusted = config.trustedRoleId ? member.roles.cache.has(config.trustedRoleId) : true;

      // --- Country Roles (require Trusted if configured) ---
      const egyptId = await this.wareraService.getEgyptCountryId();
      const belongsToEgypt = profile.country === egyptId;

      if (belongsToEgypt && isTrusted) {
        ...
        logger.info(logCtx, `Member ${member.user.tag} lacks the trusted role. Government/leadership roles will be skipped.`);
      }
